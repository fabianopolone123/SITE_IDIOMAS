...
class ReviewState(models.Model):
    ...
    def schedule(self, grade, immediate_again=False):
        ...
        if grade == self.AGAIN:
            ...
            self.due_at = now if immediate_again else now + timedelta(minutes=2)
            return
...
class VanRegistration(models.Model):
    ...
    STATUS_CHOICES = [
        ...
        (SIGNED_RECEIVED, 'Inscrição da van feita'),
    ]

    public_id = models.UUIDField(default=uuid.uuid4, editable=False, unique=True)
    responsible_name = models.CharField('nome do responsável', max_length=180)
    responsible_rg = models.CharField('RG do responsável', max_length=40, blank=True, default='')
    ...
    responsible_phone_alt = models.CharField('telefone alternativo', max_length=30, blank=True, default='')
    responsible_email = models.EmailField('email', blank=True)
    minor_name = models.CharField('nome do menor', max_length=180)
    minor_birth_date = models.DateField('data de nascimento do menor', db_index=True, null=True, blank=True)
    minor_document = models.CharField('RG/CPF do menor', max_length=60, blank=True)
    minor_cpf = models.CharField('CPF do adolescente', max_length=20, blank=True, default='', db_index=True)
    event_name = models.CharField('evento', max_length=180, default='Passaporte: Sua Identidade em Alta Definição')
    event_start_date = models.DateField('data inicial do evento', null=True, blank=True)
    event_end_date = models.DateField('data final do evento', null=True, blank=True)
    health_info = models.TextField('informações importantes de saúde', blank=True, default='')
    city = models.CharField('cidade', max_length=120, blank=True, default='São Carlos')
    signature_date = models.DateField('data do termo', null=True, blank=True)
